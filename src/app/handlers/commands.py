...
START_TEXT_TEMPLATE = (
    ...
    "🖼️ Если фото: смотрю на размер изображения, предлагаю удобную сетку и нарезаю картинку. Отступы добавляю только по краям и беру из ваших настроек (по умолчанию уровень {default_padding} — без рамки). Готовый пак загружаю в Telegram (нужен Premium). Изменить уровень можно командой /padding, например /padding padding=2 (умеренная рамка).\n\n"
